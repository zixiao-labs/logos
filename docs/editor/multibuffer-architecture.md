...
当前已落地两块可运行切片：Git 面板可打开“未提交的更改”多文件 Diff；项目文本搜索、Problems、LSP 层级结果以及显式的引用/定义动作可以打开通用 multibuffer tab。通用表面支持 excerpt 合并、准确源位置、上一项/下一项、折叠、双击回源和按视口延迟挂载 Monaco。它们验证了消费者与 UI 生命周期，**还不具备 Zed 的统一光标、跨文件撤销和 excerpt 级暂存语义**。

## 已落地的导航 UX

- 搜索侧栏保留轻量预览，并提供“在编辑器中打开结果”；Enter 同样部署当前结果，避免侧栏承担大结果集编辑器的职责。
- Problems 面板和 LSP 结果弹窗都提供 multibuffer 入口；普通 Monaco 编辑器的命令/右键菜单增加引用、定义和类型定义入口。
- excerpt 分隔线是主要导航锚点：显示工作区相对路径、源行、结果类型和摘要；路径或代码区双击会打开准确源范围。
- 工具栏提供上一个/下一个 excerpt、打开当前源文件和全部折叠；大结果集只为接近视口的 excerpt 创建 Monaco view。
- 项目文本搜索在主进程中执行，跳过依赖、构建目录、二进制和超大文件，并为单次结果设置硬上限。

通用表面当前刻意只读：excerpt 使用独立的临时 Monaco model 做语法展示，源文件仍是唯一可写事实来源。下一阶段完成 BufferRegistry 之后，才能把 excerpt 改为共享真实文件 model，而不会造成 dirty、LSP、外部变更和关闭模型之间的生命周期冲突。
